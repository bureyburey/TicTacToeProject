...
public class Board {
	/*
	 * The board is represented as a one dimensional array which simulates SIZE*SIZE grid
	 * 0 - blank
	 * 1 - 'X'
	 * 2 - 'O'
	 * examples for game variations:
	 * Winning Streak of 5:
	 * 				SIZE = 10
	 * 				DIV_FACTOR = 2
	 * winning is achieved by getting a row/column/diagonal of 5 of the same pieces
	 * Standard 3x3 Tic Tac Toe:
	 * 				SIZE = 3
	 * 				DIV_FACTOR = 1
	 * winning is achieved by getting an entire row/column/(main/secondary) diagonal of the same piece
	 */
	private int[] board;
	private final int SIZE = 10; // sets the row/column length
	private int DIV_FACTOR = 2;
	...
	public boolean indexEmpty(int index){
		/*
	...
	public boolean attemptMove(int index, Player player){
	...
	public void setIndex(int index, Player player){
		/*
	...
	public boolean willWinBlock(int index, int playerNum){
		/*
	...
	public int countColumn(int index, int playerNum){
		...
		while(runner < SIZE*SIZE){ // run on the column and count player pieces
			if(board[runner] == playerNum)
				count++;
	...
	public int countColumnStreak(int index, int playerNum){
		...
		while(runner >=0 && board[runner] == playerNum){ // run on the column and count player streak UPWARD
			if(board[runner] == playerNum)
				count++;
		...
		while(runner < SIZE*SIZE && board[runner] == playerNum){ // run on the column and count player streak DOWNWARD
			if(board[runner] == playerNum)
				count++;
			runner+=SIZE; // advance to next row (same column)
		}
